...
The tools are varied in their approaches and coverage, but tend to be focused on grammar versus usage and style, indiscriminate in choosing sources of advice, closed source or abandonware, and riddled with false positives. Every tool suffers from the problem of imprecision when compared to Proselint on our corpus (as defined by lintscore). All of the tools – including proselint – are incomplete; none of them currently free all of our collective knowledge about best practices in writing from its bindings. This fact highlights why Proselint's open-source, community participation model is vital to its continued growth. Some tools are inaccessible in that they either cost money, are closed source, or are not extensible. Proselint has none of these limitations
